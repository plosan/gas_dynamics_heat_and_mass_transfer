...
TA0 = 30;           % Average temperature of fluid A    [ºC]
TB0 = 30;           % Average temperature of fluid B    [ºC]

DeltaTA = 20;       % Amplitude of temperature oscillation of fluid A   [ºC]
DeltaTB = 5;        % Amplitude of temperature oscillation of fluid A   [ºC]

fA = 1/3600;        % Oscillation frequency for fluid A     [1/s]
fB = 1/(24*3600);   % Oscillation frequency for fluid B     [1/s]

TA = @(t) TA0 + DeltaTA*sin(2*pi*fA*t);    % Fluid A temperature   [ºC]
TB = @(t) TB0 + DeltaTB*sin(2*pi*fB*t);    % Fluid B temperature   [ºC]
...
% hold on;
% scatter(r_node, 0.5+zeros(N+2,1), 10, 'b', 'filled');
% scatter(r_surf, 0.6+zeros(length(r_surf),1), 10, 'r', 'filled');
% ylim([0.4 1.7]);
...
while n < M+1
    ...
    while error > delta
        ...
        QPn = alphaA(T(1,n))*(TA((n-1)*t_step)-T(1,n))*S(1)+lambda(T(2,n))*(T(2,n)-T(1,n))*S(1)/d(1);
        b(1) = beta*alphaA(T_est(1))*TA(n*t_step)*S(1)+(1-beta)*QPn;
        % Internal nodes 2 <= i <= N+1
        for i = 2:N+1
            lambda_w = d(i-1)/((r_surf(i-1)-r_node(i-1))/lambda(T_est(i-1)) + (r_node(i)-r_surf(i-1))/lambda(T_est(i)));
            A(i,1) = beta*lambda_w*S(i-1)/d(i-1);
            lambda_e = d(i)/((r_surf(i)-r_node(i))/lambda(T_est(i)) + (r_node(i+1)-r_surf(i))/lambda(T_est(i+1)));
            A(i,3) = beta*lambda_e*S(i)/d(i);
            A(i,2) = A(i,1) + A(i,3) + rho*V(i)*cp(T_est(i))/t_step;

            lambda_w = d(i-1)/((r_surf(i-1)-r_node(i-1))/lambda(T(i-1,n)) + (r_node(i)-r_surf(i-1))/lambda(T(i,n)));
            lambda_e = d(i)/((r_surf(i)-r_node(i))/lambda(T(i,n)) + (r_node(i+1)-r_surf(i))/lambda(T(i+1,n)));
...
% 5.4 T plot for different instants of time
t_plot = [0:900:3600]+1;
