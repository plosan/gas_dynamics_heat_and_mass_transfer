...
N1 = 35;            % Number of CV in x direction for M1 and M3
...
filename = sprintf("plots/influencia/esquema/esquema_%d.pdf", 12);


title_str = sprintf("Hi");
if beta == 1
    title_str = sprintf("\\textbf{Mapa de temperatures en $t = %d \\ \\mathrm{s}$, $\\Delta t = %.2f \\ \\mathrm{s}$, impl\\'icit}", t_max, t_step);
elseif beta == 0.5
    title_str = sprintf("\\textbf{Mapa de temperatures en $t = %d \\ \\mathrm{s}$, $\\Delta t = %.2f \\ \\mathrm{s}$, Crank--Nicolson}", t_max, t_step);
end
