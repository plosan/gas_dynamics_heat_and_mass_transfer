...
% Different time steps to analyze
t_step_vec = [0.5 1 5 10 20 50];

for t_step_pos = 1:length(t_step_vec)
...
t_step = t_step_vec(t_step_pos);    % Time step     [s]
...
filename = sprintf("plots/influencia/pas_temps/pas_temps_%d.pdf", 12+t_step_pos);
...
h = figure(t_step_pos);
hold on;
title_str = sprintf("\\textbf{Mapa de temperatures en $t = %d \\ \\mathrm{s}$, pas de temps $\\Delta t = %.2f \\ \\mathrm{s}$}", t_max, t_step);
title(title_str);
